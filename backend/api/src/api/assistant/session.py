...
from .tools import TOOLS, TOOL_MAP, READ_ONLY, WRITE_ACCESS, get_tool_definition

MODEL_NAME = "llama3.1:8b" # Or any model that supports tool calling

class AssistantSession:
    def __init__(self, user_id: str, history: List[ChatMessage], on_update: Callable[[Dict], Awaitable[None]]):
    ...
    async def confirm_tool(self, tool_call_id: str):
        if not self.pending_tool_call or self.pending_tool_call["id"] != tool_call_id:
            await self.on_update({"type": "error", "message": "No pending tool call found or ID mismatch."})
            return
