...
async def test_register_user_success(async_client, init_mock_db):
    with patch("api.routes.auth.User.find_one", new_callable=AsyncMock) as mock_find_one:
        mock_find_one.return_value = None
        with patch("api.routes.auth.User.insert", new_callable=AsyncMock) as mock_insert:
...
async def test_login_success(async_client, init_mock_db):
    with patch("api.routes.auth.authenticate_user", new_callable=AsyncMock) as mock_auth:
        mock_user = AsyncMock()
...
async def test_refresh_token_success(async_client, init_mock_db):
    with patch("api.routes.auth.jwt.decode") as mock_decode:
        ...
        with patch("api.routes.auth.User.find_one", new_callable=AsyncMock) as mock_find:
            mock_user = AsyncMock()
            mock_user.username = "testuser"
