...
def test_tool_registry_list_unregister():
    ...
    registry.register_tool(
        tool1, name="t1", description="d1", tool_type="read_only", parameters={}
    )
    registry.register_tool(
        tool2, name="t2", description="d2", tool_type="write_access", parameters={}
    )
...
async def test_automatic_metadata_extraction():
    ...
    assert (
        tool.parameters["properties"]["pipeline_id"]["description"]
        == "The ID of the pipeline."
    )
    assert tool.parameters["properties"]["count"]["type"] == "number"
    assert (
        tool.parameters["properties"]["count"]["description"]
