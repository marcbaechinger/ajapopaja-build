...
async def test_search_tasks_by_keyword(init_mock_db):
    ...
    await Task(
        ...
        spec="PostgreSQL config",
        design_doc="Use standard schema",
        pipeline_id=pid,
    ).insert()
    await Task(
        title="UI Layout", spec="Sidebar and main content", pipeline_id=pid
    ).insert()
...
async def test_search_tasks_by_status(init_mock_db):
    ...
    tasks, total = await task_queries.search_tasks(
        statuses=[TaskStatus.CREATED, TaskStatus.IMPLEMENTED]
    )
    assert total == 2
