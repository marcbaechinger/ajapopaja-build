...
async def test_complete_task_version_mismatch(init_mock_db):
    ...
    task = Task(
        title="Test Task", pipeline_id=str(pipeline.id), status=TaskStatus.INPROGRESS
    )
    await task.insert()
...
async def test_complete_task_failed_verification(init_mock_db):
    ...
    updated_task = await task_queries.complete_task(
        task_id=str(task.id), version=task.version, commit_hash="", completion_info=""
    )
...
async def test_complete_task_missing_required_design_doc(init_mock_db):
    ...
    assert updated_task.verification["success"] is False
    assert any(
        "Missing design_doc" in err for err in updated_task.verification["errors"]
    )
...
async def test_update_task_design_doc(init_mock_db):
    ...
    updated_task = await task_queries.update_task_details(
        task_id=str(task.id), version=task.version, design_doc="New design doc"
    )
...
async def test_manual_failure_override(init_mock_db):
    ...
    task = Task(
        title="Task 1", pipeline_id=str(pipeline.id), status=TaskStatus.INPROGRESS
    )
    await task.insert()

    # Manual fail
    updated_task = await task_queries.update_task_status(
        str(task.id), TaskStatus.FAILED, task.version
    )
...
async def test_transition_inprogress_to_scheduled(init_mock_db):
    ...
    updated_task = await task_queries.update_task_status(
        str(task.id), TaskStatus.SCHEDULED, task.version
    )
