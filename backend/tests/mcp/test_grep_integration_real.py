...
async def test_grep_integration_positive(real_workspace_pipeline):
    ...
    result = await grep(
        str(real_workspace_pipeline.id), "PipelineClient", file_extension="*.ts"
    )
...
async def test_grep_integration_negative(real_workspace_pipeline):
    ...
    result = await grep(
        str(real_workspace_pipeline.id), "NonExistentString12345", file_extension="*.ts"
    )
...
async def test_grep_integration_case_insensitive(real_workspace_pipeline):
    ...
    result = await grep(
        str(real_workspace_pipeline.id),
        "pipelineclient",
        file_extension="*.ts",
        ignore_case=True,
    )
...
async def test_grep_integration_truncation(real_workspace_pipeline):
    ...
    result = await grep(
        str(real_workspace_pipeline.id), "BaseClient", file_extension="*.ts"
    )
...
async def test_grep_integration_large_file(real_workspace_pipeline):
    ...
    result = await grep(
        str(real_workspace_pipeline.id), "Match BaseClient", file_extension="*.ts"
    )
    ...
    for i in range(10):
        ...
        assert (
            f"// Match BaseClient at line {line_num}" in result["matches"][i]["match"]
        )
...
async def test_grep_integration_very_large_file(real_workspace_pipeline):
    ...
    result = await grep(
        str(real_workspace_pipeline.id), "Target match", file_extension="*.ts"
    )
