...
async def test_ensure_running_starts_process(init_mock_db):
    pipeline = Pipeline(
        name="Test Pipeline", workspace_path="/tmp/test_ws", manage_vibe=True
    )
    await pipeline.insert()
    pipeline_id = str(pipeline.id)

    with patch("subprocess.Popen") as mock_popen, patch("builtins.open", MagicMock()):
...
async def test_ensure_running_skips_when_disabled(init_mock_db):
    pipeline = Pipeline(
        name="Test Pipeline", workspace_path="/tmp/test_ws", manage_vibe=False
    )
    await pipeline.insert()
    pipeline_id = str(pipeline.id)
...
async def test_stop_all(init_mock_db):
    pipeline1 = Pipeline(
        name="Test Pipeline 1", workspace_path="/tmp/test_ws", manage_vibe=True
    )
    pipeline2 = Pipeline(
        name="Test Pipeline 2", workspace_path="/tmp/test_ws", manage_vibe=True
    )
